...
function calculateHypotenuse(a, b) {
  ...
  if (typeof a === "string" || typeof b === "string") {
    console.log("Помилка, параметр має бути числом.");
    return null;
  }
...
function calculateVolumeCylinder(radius, height) {
  ...
  if (typeof radius !== "number" || typeof height !== "number") {
    console.log("Помилка, параметри мают бути числами.");
    return null;
  }
  ...
  const volumeCylinder = Math.ceil(Math.PI * radius * radius * height);
...
function getFractionalPart(num) {
  ...
  const fractional = num - part;
  return Math.fround(fractional);
}
...
function estimateInvestment(principal, interestRate, years) {
  ...
  const sumInvestment = principal * Math.pow(1 + interestRate, years);
  return Math.round(sumInvestment);
}
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  if (!Array.isArray(products)) {
    ...
    console.log("повідомлення про помилку");
    // Повертаємо null, що свідчить про неможливість обробки вхідних даних.
    return null;
  }
